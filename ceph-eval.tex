...
The initial stability issues mentioned in chapters (X,Y,Z) are fixed by
migrating from Ceph version 0.48/0.55 to 0.64, the latest stable version at the
...
data pool used by previous Ceph version were set to 2x replication at the
minimum. This potentially halved the write performance. With version 0.64 we
now had the ability to set the replication level to 1, which is the preferred
value for a HPC environment like ours running on high-end and reliable storage
backend hardware (e.g. DDN SFA10K). 
...
We upgraded our test systems with kernel version 3.9 and performed a
RADOS Bench test.  The
results were extremely positive and presented in Figure~\ref{fig:rados-kernel-35vs39}.
...
\item Within a single sharing mode (either shared or unique directory), stat
...
creation operation, it is likely due to lack of workload stress on MDS.
...
disabled replication for pools, but this will take a toll somewhere in the
processing and we do not know yet if this would be significant.
